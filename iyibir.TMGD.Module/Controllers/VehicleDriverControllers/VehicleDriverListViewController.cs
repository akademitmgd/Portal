				...
				if (employee.EmployeeType == EmployeeType.Customer)
				{
					...
					if (customer != null)
					{
						if (!string.IsNullOrEmpty(customer.UETDSUsername) && !string.IsNullOrEmpty(customer.UETDSPassword))
						{
							...
							meslekiYeterlilikSorgulaRequest request = new meslekiYeterlilikSorgulaRequest();
							request.wsuser = UETDSProvider.GetWsUser(customer.UETDSUsername, customer.UETDSPassword);
							request.kimlikNo = driver.TCKN;

							uetdsMesSorguSonuc sonuc = UETDSProvider.meslekiYeterlilikSorgula(client, request);

							//uetdsMesSorguSonuc sonuc = new UETDSHelper.UETDSHelper(customer.UETDSUsername,customer.UETDSPassword).MeslekiYeterlilikSorgula(driver.TCKN);
							if (sonuc != null)
							{
