...
namespace iyibir.TMGD.WebAPIV2.Models
{
    public partial class VehicleDriver
    {
        ...
        public DataResult GetObjectById(Guid CustomerOid,string TCKN)
        {
            ...
            {
                List<VehicleDriver> items = new List<VehicleDriver>();
                string commandString = string.Format("SELECT * FROM VehicleDriver WHERE Customer = '{0}' and TCKN = '{1}'", CustomerOid,TCKN);
                using (SqlConnection cnn = new SqlConnection(connectionString))
                {
                    cnn.Open();
                    using (SqlCommand cmd = new SqlCommand(commandString, cnn))
        ...
        public DataResult GetObjects(Guid customerOid)
        {
            ...
            {
                List<VehicleDriver> items = new List<VehicleDriver>();
                string commandString = string.Format("SELECT * FROM VehicleDriver WHERE Customer = '{0}'",customerOid);
                using (SqlConnection cnn = new SqlConnection(connectionString))
                {
                    cnn.Open();
                    using (SqlCommand cmd = new SqlCommand(commandString, cnn))
