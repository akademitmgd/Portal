...
namespace iyibir.TMGD.WebAPIV2.Models
{
    public partial class PackingGroup
    {
        ...
        public DataResult GetObjectByHazardousGoods(Guid HazardousGoodsOid)
        {
            ...
            {
                List<PackingGroup> items = new List<PackingGroup>();
                string commandString = string.Format(@"SELECT PackingGroups FROM PackingGroupPackingGroups_HazardousGoodsHazardousGoods 
                ...
                using (SqlConnection cnn = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(commandString, cnn))
                    {
                        ...
                        while (dr.Read())
                        {
                            PackingGroup packingGroup = dr["PackingGroups"] == DBNull.Value ? null : ((List<PackingGroup>)new PackingGroup().GetObjectById(Guid.Parse(dr["PackingGroups"].ToString())).Data).FirstOrDefault();
