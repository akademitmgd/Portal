...
namespace iyibir.TMGD.WebAPIV2.Models
{
    public partial class Vehicle
    {
        ...
        public DataResult GetObjectById(Guid CustomerOid,string Plate)
        {
            ...
            {
                List<Vehicle> items = new List<Vehicle>();
                string commandString = string.Format("SELECT * FROM Vehicle WHERE Customer = '{0}' and Plate = '{1}'", CustomerOid,Plate.Trim());
                using (SqlConnection cnn = new SqlConnection(connectionString))
                {
                    cnn.Open();
                    using (SqlCommand cmd = new SqlCommand(commandString, cnn))
        ...
        public DataResult GetObjects(Guid CustomerOid)
        {
            ...
            {
                List<Vehicle> items = new List<Vehicle>();
                string commandString = string.Format("SELECT * FROM Vehicle WHERE Customer = '{0}'",CustomerOid);
                using (SqlConnection cnn = new SqlConnection(connectionString))
                {
                    cnn.Open();
                    using (SqlCommand cmd = new SqlCommand(commandString, cnn))
